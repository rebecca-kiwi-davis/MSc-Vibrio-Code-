...
epi_curve_plot <- ggplot(plot_counts, aes(x = plot_date, y = case_counts)) +
  ...
  # Filters labels
  # This expands the timeline to full year edges even if your CSV completely lacks Jan or Dec rows.
  scale_x_date(
    breaks = make_date(2020, c(1, 4, 7, 10), 1),
    labels = c("Jan", "Apr", "Jul", "Oct"),#Sets the text labels for those specific quarterly markers.
    limits = c(make_date(2020, 1, 1), make_date(2020, 12, 31)),#axis covers entire 12 months
    expand = c(0, 0) #remove white space at either side of plot
  ) +
