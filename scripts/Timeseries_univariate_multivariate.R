...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
# 18. Time series
#biological thresholds (>18°C for SST) and local
...
#make calender and process case and environ data
build_region_ts <- function(current_area) {
  ...
  # Remove any non grouped columns
...
ts_screen_configs <- list(
  ...
  list(prefix = "SSS_lag",   label = "Sea Surface Salinity (Local 1 SD Anomaly)", type = "standardized"),
  list(prefix = "Pmean_lag", label = "Precipitation (Local 1 SD Anomaly)", type = "standardized")
)
...
# Output table layout (3 parameters + 4 lags)
...
doc_ts_screen <- read_docx() %>%
  body_add_par("Table TS.1:  Time Series Univariate Models", style = "heading 1") %>%
...
print(doc_ts_screen, target = "MSc_Thesis_Table_TimeSeries_Screening.docx")
cat("\n[SUCCESS]:  time series  table saved.\n")

# ===================================================================
# STEP 3: Area-Specific, Single-Lag Multivariate Time Series Model
# biological thresholds (>18°C for SST) and local
# regional standardization (1 SD Z-scores for SSS and Pmean) to ensure
# fair comparison across areas.
# -------------------------------------------------------------------
...
for (current_area in ts_areas) {
  ...
  # calc the baseline standard deviations for specific regions
  sd_sss   <- sd(d$SSSmean, na.rm = TRUE)
  sd_pmean <- sd(d$Pmean, na.rm = TRUE)

  # Iterate through each lag day independently (0 to 3)
  for (current_lag in 0:ts_lag_max) {

    # 1. Build a local dataframe, applying threshold and regional SD scaling to the specific lag day
    d_lag_fit <- d %>%
      mutate(
    ...
    predictors <- list(
      ...
      list(coef_name = "sss_trans",   label = "Sea Surface Salinity (Local 1 SD Anomaly)"),
      list(coef_name = "pmean_trans", label = "Precipitation (Local 1 SD Anomaly)")
    )
...
# 4. Group results across all areas and make p-value adjustments (BH)
...
doc_ts_adjusted <- read_docx() %>%
  body_add_par("Table TS.2: Multivariate Time Series Model with Lags (Lags 0-3)", style = "heading 1") %>%
...
cat("\n[SUCCESS]: Standardised area-specific, single-lag table.\n")

# -------------------------------------------------------------------
# STEP 4: area specifics
# -------------------------------------------------------------------
...
for (current_area in ts_areas) {
  ...
  # make Multipanel Lag Response figures
  ...
  plot_single_lags(region_models, ts_lag_max, "sss",
                   paste0(toupper(current_area), ": SSS (1 SD )"), "RR (per 1 Local SD increase)")
  plot_single_lags(region_models, ts_lag_max, "pmean",
                   paste0(toupper(current_area), ": Precipitation (1 SD )"), "RR (per 1 Local SD increase)")
